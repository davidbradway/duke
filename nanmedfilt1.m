...
%NANMEDFILT1  One dimensional median filter.
%   Y = NANMEDFILT1(X,N) returns the output of the order N, one dimensional
...
%   then NANMEDFILT1 operates along the columns of X.
%
%   If you do not specify N, NANMEDFILT1 uses a default of N = 3.
%   For N odd, Y(k) is the median of X( k-(N-1)/2 : k+(N-1)/2 ).
%   For N even, Y(k) is the median of X( k-N/2 : k+N/2-1 ).
%
%   Y = NANMEDFILT1(X,N,BLKSZ) uses a for-loop to compute BLKSZ ("block size") 
%   output samples at a time.  Use this option with BLKSZ << LENGTH(X) if 
%   you are low on memory (NANMEDFILT1 uses a working matrix of size
%   N x BLKSZ).  By default, BLKSZ == LENGTH(X); this is the fastest
%   execution if you have the memory for it.
%
%   For matrices and N-D arrays, Y = NANMEDFILT1(X,N,[],DIM) or 
%   Y = NANMEDFILT1(X,N,BLKSZ,DIM) operates along the dimension DIM.
%
...
if ~isempty(DIM) && DIM > ndims(x)
	error(message('signal:nanmedfilt1:InvalidDimensions'))
end
...
% Call nanmedfilt1D (vector)
for i = 1:prod(siz(2:end)),
	y(:,i) = nanmedfilt1D(x(:,i),n,blksz);
end
...
%                       Local Function
%-------------------------------------------------------------------
function y = nanmedfilt1D(x,n,blksz)
%nanmedfilt1D  One dimensional median filter.
%
% Inputs:
%   x     - vector
...
for i=1:blksz:nx
    ...
    y(i:min(i+blksz-1,nx)) = nanmedian(xx,1);
end
